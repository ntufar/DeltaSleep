...
fun TrendsScreen(
    ...
    Column(
        ...
        val month = remember(d) { d.nights.filter { it.startMs >= cutoff } }

        Text(
            "Based on ${d.nights.size} nights (last 90 days)",
        ...
        Spacer(Modifier.height(8.dp))
        WeekdayHeatmap(avgs = remember(d) { weekdaySnoreAvg(d.nights, zone) })
